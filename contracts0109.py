...
class MappingEntry(BaseModel):
    ...
    # Another target claims this same source column at higher confidence. Not
    # necessarily wrong — one column can legitimately feed two attributes — but
    # it is the signal the semantic tier uses to decide whether this target
    # actually has a source at all.
    contested_with: Optional[str] = None
    # The independent semantic view of this target, carried so a later pass can
    # act on it: whether the model looked at all, and which source it chose.
    llm_has_view: Optional[bool] = None
