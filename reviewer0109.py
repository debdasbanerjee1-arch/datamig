...
def apply_decisions(spec: MappingSpec, decisions: dict,
    ...
    src_label = _source_label(spec)
    # An unmapped target now also appears in spec.mappings as a SOURCELESS
    # reject row, so the reviewer sees every attribute. That row is a
    # presentation placeholder, not a mapping: it must be replaced here when the
    # target is promoted to a load-time default, or the default is silently
    # dropped and the column loads empty. Only rows that actually name a source
    # count as "already mapped".
    placeholders = {m.target_attribute: m for m in spec.mappings
                    if not m.source_attributes and not m.transformation_sql}
    mapped_names = {m.target_attribute for m in spec.mappings
                    if m.target_attribute not in placeholders}
    still_unmapped = []
    for u in spec.unmapped_target:
        attr = u["attribute"]
        ...
        srcs: list[str] = []
        if action == "reject":
            # Deliberate exclusion, not migrated — and the placeholder row STAYS
            # so the reviewer's decision is visible in the report rather than the
            # attribute silently disappearing from it.
            continue
        if attr in placeholders:          # a real row replaces it below
            spec.mappings = [m for m in spec.mappings if m is not placeholders[attr]]
