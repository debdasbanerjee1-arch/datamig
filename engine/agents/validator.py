...
def validate_spec(spec: MappingSpec, target_dict: dict, insight: TableInsight,
    ...
    add("source_refs_exist", "wellformed", "fail" if bad_ref else "pass",
        f"mappings referencing unknown columns: {bad_ref}" if bad_ref else "all source refs valid.",
        sev="hard")
    add("transforms_compile", "wellformed", "fail" if no_compile else "pass",
        f"transforms failing to compile: {no_compile}" if no_compile else "all transforms compile.",
        sev="hard")
    ...
    add("transforms_execute", "wellformed", "fail" if no_execute else "pass",
        (f"transforms failing on the actual data (wrong source?): {no_execute}"
         if no_execute else "all transforms execute on the staged data."),
        sev="hard")
    ...
    cols_sql = ", ".join(f'{m.transformation_sql} AS "{m.target_attribute}"' for m in executable)
